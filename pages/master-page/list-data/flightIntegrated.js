...
import { ModalEditForm } from "@/components/pages/list-data/modalEditFormIntegratedFlight";
...
import { handleFetchFlightDatatable } from "app/api/helper";
...
const DataTable = (props) => {
  ...
  const handleGetListData = useCallback(
    async (updatedCurrentPage, updatedPageSize, updatedDataFilters) => {
      ...
      const { resultData, pagination, isError } =
        await handleFetchFlightDatatable(payload);
      if (isError) {
  ...
  const handleEditData = async (dataForm) => {
    ...
    const { result, error } = await fetchUpdateData(payload);
    if (result) {
      if (result.statusCode === 200) {
        alert = {
          isSuccess: true,
