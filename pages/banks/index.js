...
import { formFilterScheme, columnTableScheme } from "@/components/pages/bank/list-data/bankScheme";
import { NotificationManager } from "react-notifications";
import CircularProgress from "@/components/CircularProgress";
import { ModalUpdateStatus } from "@/components/pages/bank/list-data/modalUpdateStatus";
import { ModalEditForm } from "@/components/pages/bank/list-data/modalEditForm";
import { Pagination } from "antd";
import { ModalCreateForm } from "@/components/pages/bank/list-data/modalCreateForm";
import DynamicFilter from "@/components/Form/DynamicFilter";
import { FilterOutlined, FilterFilled } from "@ant-design/icons";
import { handleFetchBankDatatable, handleDeleteBank, handleManageBank } from "app/api/helper";
...
const ListData = (props) => {
  ...
  const handleGetListData = useCallback(
    async (updatedCurrentPage, updatedPageSize) => {
      ...
      await handleFetchBankDatatable(payload);
  ...
  const fetchUpdateStatus = async (data) => {
    ...
    let { result, error } = await handleDeleteBank(payload);
    if (!error) {
  ...
  const handleEditData = async (dataForm) => {
    let alert = "";
    setIsModalEditFormShow({
      isLoadingPromise: true,
      data: isModalEditFormShow.data,
    });
    let { result, error } = await fetchUpdateData(dataForm);
    if (result) {
      if (result.statusCode === 200) {
        alert = {
          isSuccess: true,
  ...
  const handleCreateData = async (formData) => {
    ...
    const { result, error } = await handleManageBank(formData);
    if (result) {
      if (result.statusCode === 200) {
        alert = {
          isSuccess: true,
