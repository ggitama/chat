...
import { MainLoader, TabNav} from "@/components/pages/bookings/hotel/main"
...
import { handleFetchHotelDetail } from "app/api/helper"
import { NotificationManager } from "react-notifications"
import { SectionDetail } from "@/components/pages/bookings/hotel/sectionDetail"
import { SectionPassenger } from "@/components/pages/bookings/hotel/sectionPassenger"
...
const ScreenPage = (props)=>{
  ...
  const pageTitle = "Hotel Transaction Detail"
  ...
  const handlerGetListData = useCallback(
    ...
    const {isError,resultData,pagination} = await handleFetchHotelDetail(payload)
    if(isError){
    ...
    // setTimeout(()=>{
      ...
      resultData.type = 'received-hotel-issued';
        ...
        showLoader ? (<MainLoader heightTable={heightTable}/>) :(
          <>
            <TabNav props={[...tabNavProps]}>
              ...
              <SectionPassenger key="guest" dataSource={contentData}/>
            </TabNav>
          </>
        )
      }
