...
export const useMainHooks = (state, dispatch) => {
  const handleGetListData = useCallback(async (params = {}) => {
    ...
    let { isError, pagination, resultData } = await BookingApi.fetchBankDatatable(payload)
  ...
  const handleModalDeleteSubmit = async (formData, callback) => {
    let alert = "";
    dispatch({
      type: "IS_SHOW_MODAL_DELETE", data: {
        isPromise: true,
        data: formData
      }
    })
    ...
    const { resultData, isError } = await BookingApi.deleteBank(
      payload
    );
    let methodNotif = (isError) ? "error" : "success"
    let message = (isError) ? isError : "Success Delete Bank"
  ...
  const handleModalEditSubmit = async (formData, callback) => {
    try {
      ...
      if (formPayload.logo.includes('http')) {
        delete formPayload.logo
      }
      let payload = {
        ...formPayload,
        uuid: dataFromState.uuid
      }
      let { isError, resultData } = await BookingApi.manageBank(payload)
      let methodNotif = (isError) ? "error" : "success"
      let message = (isError) ? isError : "Success edit bank"
  ...
  const handleModalCreateSubmit = async (formData, callback) => {
    try {
      ...
      let { isError, resultData } = await BookingApi.manageBank(payload)
      let methodNotif = (isError) ? "error" : "success"
      let message = (isError) ? isError : "Success create bank"
