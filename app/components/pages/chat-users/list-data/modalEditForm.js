...
import { modalEditScheme } from "./bannerScheme";
...
export const ModalEditForm = (props) => {
  ...
  const handleFormUpdate = (index, value) => {
    ...
    switch (tempDataForm[index].type) {
      ...
      default:
        ...
        if (formFieldValidation(validationType, value)) {
          tempDataForm[index].value = value;
  ...
  const handleGetVoucherList = async(productType, id = null) => {
    ...
    tempForm.promoId.value = id ? isModalShow.data.promoId : "" 
    await setVoucherList(temp)
    await setFormData(tempForm)
  }

  const handleGetDetailData = useCallback(async () => {
    let temp = {...isModalShow}
    // temp.data.promoId = formData.promoId.value
    const updateDataForm = modalEditScheme( voucherList, temp )
    setIsLoading(false);
    setFormData(updateDataForm);
    setImageUrl({
      homeImageUrl: isModalShow.data.homeImageUrl,
      detailsImageUrl: isModalShow.data.detailsImageUrl
    })
  }, [isModalShow]);
  ...
  useEffect(() => {
    if (isModalShow && !isModalShow.isLoadingPromise) {
      setIsLoading(true);
      handleGetVoucherList(isModalShow.data.productType, 1)
      handleGetDetailData();
    }
  }, [handleGetDetailData]);
  return(
              ...
              <div className="gx-w-100 gx-flex gx-flex-row gx-mt-4">
                ...
                <DatePicker 
                  ...
                  value={
                    ...
                    moment(formData.startDate.value, 'DD-MM-YYYY') : ""}
              ...
              <div className="gx-w-100 gx-flex gx-flex-row gx-mt-4">
                ...
                <DatePicker 
                  ...
                  value={
                    ...
                    moment(formData.endDate.value, 'DD-MM-YYYY') : ""}
                  onChange={ (date, dateString) => {
